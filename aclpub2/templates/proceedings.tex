...
\BLOCK{for date, pages in program}
  ...
  \BLOCK{for page in pages}
    ...
    \BLOCK{for entry in page}
      ...
      \textbf{\VAR{session_times(entry)}} & \emph{\textbf{\VAR{entry.title}}}\\\\
      \BLOCK{endif}

      \BLOCK{if entry.type is equalto("paper")}
        \BLOCK{set paper = id_to_paper[entry.paper.id]}
        \VAR{entry.start_time} - \VAR{entry.end_time} & \hyperlink{page.\VAR{paper.start_page}}{\emph{\VAR{paper.title}}}\\
...
% \BLOCK{set program_title_printed = false}  % control variable

% \BLOCK{for date, pages in program}
%   \BLOCK{set ns = namespace(first=true)}

%   \BLOCK{for page in pages}
%     \BLOCK{if not program_title_printed}
%       \begin{center}
%         {\Large \textbf{Program}}
%       \end{center}
%       \vspace*{0.5em}
%       \BLOCK{set program_title_printed = true}
%     \BLOCK{endif}
