...
\BLOCK{for block in program_committee}
  ...
  \BLOCK{if block.entries[0].area}
    ...
    \BLOCK{if block.role == "Reviewers"}
      \BLOCK{for reviewer in block.entries}
        \VAR{reviewer.name|title}, \VAR{reviewer.institution}\\
      \BLOCK{endfor}
    \BLOCK{else}
...
% \BLOCK{set total_dates = program|length}
% \BLOCK{set date_index = 0}
% \clearpage
\begin{center}
  {\Large \textbf{Program}}
\end{center}
...
\BLOCK{for date, pages in program}
  % \BLOCK{set date_index = date_index + 1}
  % \BLOCK{set total_pages = pages|length}
  % \BLOCK{set page_index = 0}
  \BLOCK{set ns = namespace(first=true)}

  \BLOCK{for page in pages}
    \BLOCK{set page_index = page_index + 1}

    \begin{tabular}{p{24mm}p{124mm}}
    \multicolumn{2}{l}{\bf \VAR{program_date(date)} \BLOCK{if not ns.first}(continued)\BLOCK{endif}} \\\\
    \BLOCK{for entry in page}
      \BLOCK{if entry.type == "header"}
        \VAR{session_times(entry)} & \emph{\VAR{entry.title}}\\\\
      \BLOCK{endif}

      \BLOCK{if entry.type == "paper"}
    ...
    % Only insert \newpage if not the last page of the last date
    % \BLOCK{if not (date_index == total_dates and page_index == total_pages)}
      % \newpage
    % \BLOCK{endif}
